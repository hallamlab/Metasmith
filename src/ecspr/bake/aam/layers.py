...
# One row per ATOM correspondence -- the shape `bake_metabolism` encodes. The extractor
# emits a COMPACT shape (one row per (mnxr, element, sub, prod) with comma-joined index
# lists) because that is what a per-reaction extraction naturally produces; `explode`
# is the one place the two shapes are reconciled.
ATOM_COLS = ("mnxr", "element", "substrate", "product", "sub_idx", "prod_idx",
             "pair_w", "method", "source", "confidence")

# The 6-tuple that identifies one atom correspondence. Two layers writing the same key
# with different weights is the collision gate's subject: it means a reaction was
# claimed twice, which additive stacking is supposed to make impossible.
PAIR_KEY = ("mnxr", "element", "substrate", "product", "sub_idx", "prod_idx")

# A correspondence carried by a single member of a layer -- present, but uncorroborated.
...
# =====================================================================
# fusion WITHIN a layer -- several members, one answer
# =====================================================================

def fuse_members(members: dict, single_credit: float = SINGLE_MEMBER_CREDIT):
    """Fuse per-atom tables from several members of ONE layer.

    `members` is `{name: DataFrame}` in the ATOM_COLS shape. The arithmetic is kept as
    small as it can be and still be honest:

      consensus         every member that speaks about this source atom sends it to the
                        same destination -> full weight, one row.
      <member>_only     one member speaks -> half weight. Present but uncorroborated.
      disagree_diluted  members disagree -> ONE ROW PER DESTINATION, weight split by
                        member confidence, so a contested atom transfers LESS rather
                        than committing to a winner.

    NO CORRELATION DISCOUNT. The previous implementation multiplied a shared-architecture
    floor into a `confidence` column, and the atom graph sums `pair_w` -- it never reads
    `confidence`. So the discount was documentation with the shape of arithmetic. If
    consensus coverage later argues for one, it has to land on `pair_w` to mean anything.

    THE DIVISION GUARD IS LOAD-BEARING. `wsum` divides the disagreement weights, and a
    member with a missing confidence contributes NaN -- which is truthy, so `or` does not
    catch it and every weight in the reaction becomes NaN. Guarded explicitly below.
    """
    key = ["mnxr", "element", "substrate", "sub_idx"]
    # {(mnxr, element, substrate, sub_idx) -> {member: [(product, prod_idx, w, conf)]}}
    ...
    for k, per_member in speak.items():
        mnxr, el, sm, s_idx = k
        # each member's single best destination for this source atom
        picks = {}
        ...
        if len(picks) == 1:
            ...
            wsum = sum(v for v in confs.values() if v == v)      # NaN != NaN
            if not (wsum > 0):
...
def additive_gates(below: pd.DataFrame, added: pd.DataFrame, name: str):
    """Refuse unless `added` is genuinely additive over `below`. Four checks.

    Refuse, not warn. Each of these is a way the stack could silently stop meaning what
    it says while every table still looked well-formed, and a warning about that gets
    read once and then scrolled past for the rest of the build.
    """
    problems = []

    # 1. ADDITIVE. A layer may only claim (reaction, element) combinations nothing below
    #    it claimed. Overwriting is a replacement, which is a different and larger claim.
    ...
    # 2. NO CROSS-SOURCE COLLISION on the pair key. Two layers writing the same atom
    #    correspondence with different weights means the same atom was accounted twice.
    if len(below) and len(added):
    ...
    # 3. NO ELEMENT LOSES REACTIONS. An append that shrinks an element's coverage is not
    #    an append; it is a rewrite that happened to be spelled as one.
    if len(below):
        # Concatenated ONCE, outside the loop. Inside it, this is four full copies of a
        # million-row frame to answer four one-column questions.
    ...
    # 4. NO NEGATIVE RANKS. `CanonicalRankAtoms` is non-negative by construction, so a
    #    negative rank means a sentinel leaked into the identity space -- and the bake
    #    packs ranks as unsigned, where it would wrap silently.
    if len(added):
...
def stack(layers: list):
    """Lay `layers` down in order, each claiming only what nothing below it claimed.

    Returns (table, report). Restriction happens BEFORE the gates, so gate 1 is a real
    assertion about the restriction rather than a restatement of it: it fires when the
    restriction itself is broken (a layer that claimed a reaction under one element and
    a different one under another, say), not merely when a layer overlapped.
    """
...
def _spec(s: str):
    """`name=path[,method[,confidence]]` -- one layer or one member."""
...
def cmd_fuse(args):
    """Fuse several members of ONE layer into that layer's table."""
    members = {}
