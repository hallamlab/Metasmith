...
def expected_per_key(dep_insts: list, key_insts: list) -> int | None:
    ...
    slices are the same size. A singleton broadcast answers 1; a dependency
    with no lineage relation to the key answers None, because the positional
    fallback is an alignment convention rather than a fact about the data.
    """
