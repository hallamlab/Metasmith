...
read metadata is several facts, and the alternative is typing JSON by hand into
a field whose braces already mean `{column}`.
"""
...
def entries(row: dict) -> list[dict]:
    """What a value row holds, as `[{key, value}]`.
    ...
    if not isinstance(raw, list):
        return [{"key": "", "value": row.get("value") or ""}]
    out: list[dict] = []
    for e in raw:
        ...
        out.append({"key": str(e.get("key") or "").strip(), "value": e.get("value") or ""})
    return out
