...
def protocol(context: ExecutionContext):
    ...
    out_bam_path_container   = context.Get(out_bam).container / "alignments.sorted.bam"
    ...
    context.ExecWithContainer(
        ...
        cmd = f"""
                mkdir -p {out_bam_path_container}
                samtools view {cpus_string} -b {out_sam_path.container} | samtools sort {cpus_string} -o {out_bam_path_container}
                samtools index {out_bam_path_container}
        """
    )
    return ExecutionResult(success=context.Get(out_bam).local.exists())
