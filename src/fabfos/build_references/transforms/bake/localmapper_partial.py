"""AAM member lane: LocalMapper over the PARTIAL universe. Pass 3.

Read `localmapper_rescue.py` for the lane's shape. What differs here is only what it is
...
pairs     = model.AddProduct(lib.GetType("interm::aam_member_localmapper_partial"))
ev        = model.AddProduct(lib.GetType("evidence::tool_output"))

MEMBER = "localmapper"
TOOL = "localmapper_partial"
SHARDS = 1
...
RESOLVE = """
    ...
    if [ "$N" -ne 1 ]; then
        echo "[localmapper_partial] expected one release under {metanetx}, found $N" >&2
        exit 1
    fi
