...
"""grouping_diagnostics -- GROUPING_DIAGNOSTICS (asv_pipeline.nf:4929)

reads the pre-augmentation metadata deliberately -- it is what produces the
soft labels augmentation applies. The .nf rebinds its counts input to the
corrected matrix at asv_pipeline.nf:2915, after the call site, so that
rebinding is dead and is not ported.
