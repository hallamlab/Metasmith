...
"""spieceasi_external -- no .nf process

the `off` arm. Unlike the other off-arms this one is not a null producer:
asv_pipeline.nf:2708-2715 substitutes pre-computed graph files from disk, so
the driver has to supply them.
