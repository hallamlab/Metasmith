"""Fetch the Pratama 2026 corpus from a manifest. Resumable, size-checked, login-node safe.

A file is promoted out of staging only once its byte count matches the manifest, so an
interrupted run never leaves a truncated file that looks complete. The read files are single
objects of several gigabytes each, so a resumed transfer is the normal case rather than the
exception, and --workers is what decides how hard this leans on the source.
"""
...
def fetch_one(row: dict, root: Path, staging: Path, retries: int = 12) -> tuple[str, str]:
    want = int(row["bytes"])
    ...
    for attempt in range(retries):
        ...
        #
        # curl does its OWN retrying, and it must stay switched off here. Combined with -C -
        # it re-sends from the offset it resolved when the invocation started, so a retry
        # after a partial transfer appends to what the partial already holds. Usually that
        # overshoots and the branch below throws the partial away, which is only wasted
        # bandwidth -- but twice during the 823 GB acquisition it landed on exactly the
        # published byte count and produced a file that passed every size check and failed
        # its MD5. Retrying here instead means every attempt is a fresh curl that resolves
        # -C - against the partial as it now stands.
        rc = subprocess.run(
            ["curl", "-sS", "--fail", "--location", "--retry", "0",
        ...
        # ENA's refusals are time-limited rather than permanent, and retrying into one
        # extends it. Back off, but keep the ceiling low enough that a 12-attempt object
        # still finishes inside a pass.
