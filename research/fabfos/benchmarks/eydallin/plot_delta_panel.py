#!/usr/bin/env python3
"""Histogram of delta draw across every metabolite for any delta_panel.py run, glycogen marked.

Generalises `plot_ddg_delta_panel.py` (kept as-is) to take a gene label for the title
instead of hardcoding ddg/lpxP. See that script's docstring for the arcsinh-axis rationale.

    mamba run -n figure-net python main/benchmarks/eydallin/plot_delta_panel.py \
        --panel cache/glgC_delta_panel_fold2.0_as_written_C.parquet --gene-label "glgC"
"""
...
def main():
    ...
    p.add_argument("--panel", type=Path, required=True, help="parquet written by delta_panel.py")
    p.add_argument("--gene-label", required=True, help="e.g. 'glgC' -- used in title and axis label")
    ...
    ax.set_xlabel(f"Δ draw (glucose leak, base → {args.gene_label} ×2 pert)  —  signed arcsinh scale",
    ...
    fig.suptitle(f"Whole-metabolome $\\Delta I_{{eff}}$ under {args.gene_label} overexpression",
