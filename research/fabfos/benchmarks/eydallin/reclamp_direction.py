...
Then encode it against the bake's own vocabulary, which is what makes it loadable:
    ...
    python -m ecspr.bake.metabolism direction --direction OUT.parquet \\
        --vocab <bake>/vocab.parquet --out <bake>/direction.parquet
"""
import sys, math
...
# The bound the pinned annotation was built with. Not read from canon: canon now carries
# the NEW value, and the isolation proof below needs the old one to reproduce.
OLD_CLAMP = 100.0
...
mu = (df.lambda_shrink * df.dG_raw).to_numpy(float)      # mu_eff, pre-clamp
...
# Isolation proof: at the OLD bound this must reproduce the shipped table exactly.
