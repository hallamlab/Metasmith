#!/usr/bin/env python3
"""Gene overexpression: delta draw across every metabolite, universal ground.

Generalises `ddg_delta_panel.py` (kept as-is; its cache and the report already point at
it) to take any host-GEM reaction(s) instead of hardcoding ddg/lpxP's MNXR97903.

Two solves, glucose source, universal leak -- base (host GEM, unperturbed) and pert (the
named reaction(s) folded up). `measure_leak`'s draw dict already covers every metabolite
that became a node, so subtracting the two gives one number per metabolite: how much of
the leak that species pulled in shifted under the perturbation. This is a specificity
read (does the target stand out from everything else this perturbation moved), not a
no-op floor check.

    mamba run -n ecspr python main/benchmarks/eydallin/delta_panel.py \
        --gene glgC --rxn MNXR145050 --fold 2.0
"""
from __future__ import annotations

import argparse
import sys
from pathlib import Path

import pandas as pd

ROOT = Path(__file__).resolve().parents[4]
sys.path.insert(0, str(ROOT / "src"))

from ecspr.build import load_pairs, load_direction_ratios, graph_from_pairs  # noqa: E402
from ecspr.graph import Terminal, measure_leak                               # noqa: E402

sys.path.insert(0, str(Path(__file__).resolve().parent))
import bake_pairs  # noqa: E402

ATOM_PAIRS = bake_pairs.atom_pairs()
HOST_GEM = ROOT / "data/fabfos/benchmarks/hosts/e_coli_k12/gpr_gem.parquet"
OUT_DIR = Path(__file__).resolve().parent / "cache"
OUT_DIR.mkdir(parents=True, exist_ok=True)

SOURCE_MNXM = "MNXM1364061"    # D-glucose, as run_pilot_glycogen.py resolves it
GLYCOGEN_MNXM = "MNXM738130"   # BiGG species -- the one iML1515 (and this host GEM) carries


def main():
    p = argparse.ArgumentParser(description=__doc__.split("\n")[0])
    p.add_argument("--gene", required=True, help="label only -- used for the log line and output filename")
    p.add_argument("--rxn", action="append", required=True, metavar="MNXR",
                   help="host reaction(s) to perturb; repeatable")
    p.add_argument("--fold", type=float, default=2.0,
                   help="conductance fold-change on the named reaction(s)")
    p.add_argument("--orientation", choices=("as_written", "reversed"), default="as_written")
    p.add_argument("--element", default="C")
    p.add_argument("--leak", type=float, default=1e-6)
    args = p.parse_args()
    rxns = sorted(set(args.rxn))

    pairs = load_pairs(ATOM_PAIRS, element=args.element)
    ratios = load_direction_ratios(bake_pairs.direction_ratios())
    host = pd.read_parquet(HOST_GEM)
    base_w = {m: 1.0 for m in host["mnxr"].dropna().astype(str).unique()}

    absent = [r for r in rxns if r not in base_w]
    if absent:
        raise SystemExit(f"[{args.gene}-panel] {absent} not in the host GEM's reaction set "
                         f"-- a true addition needs --weight machinery this script doesn't "
                         f"have; use run_pilot_glycogen.py's --rxn/--weight for that case")
    pert_w = dict(base_w)
    for r in rxns:
        pert_w[r] = base_w[r] * args.fold
    print(f"[{args.gene}-panel] {rxns}: weight 1.0 -> {args.fold} (fold {args.fold})",
          file=sys.stderr)

    def solve_(weights, tag):
        g = graph_from_pairs(pairs, args.element, weights, ratios, orientation=args.orientation)
        print(f"[{args.gene}-panel] {tag}: {g.n:,} nodes / {g.m:,} edges from "
              f"{g.meta['n_reactions_used']:,} reactions (AAM gap {g.meta['n_aam_gap']:,})",
              file=sys.stderr)
        src = Terminal.metabolite(g, SOURCE_MNXM, label="source")
        if src.missing:
            raise SystemExit(f"[{args.gene}-panel] source {SOURCE_MNXM} absent from the built graph")
        r = measure_leak(g, src, [], leak=args.leak)
        return g, r

    g_base, r_base = solve_(base_w, "base")
    g_pert, r_pert = solve_(pert_w, "pert")

    base_keys, pert_keys = set(r_base["draw"]), set(r_pert["draw"])
    only_base, only_pert = base_keys - pert_keys, pert_keys - base_keys
    if only_base or only_pert:
        print(f"[{args.gene}-panel] WARNING: node sets differ -- {len(only_base)} only in "
              f"base, {len(only_pert)} only in pert", file=sys.stderr)

    rows = []
    for m in sorted(base_keys & pert_keys):
        db, dp = r_base["draw"][m], r_pert["draw"][m]
        rows.append((m, db, dp, dp - db))
    df = pd.DataFrame(rows, columns=["mnxm", "draw_base", "draw_pert", "delta"])

    if GLYCOGEN_MNXM not in set(df.mnxm):
        raise SystemExit(f"[{args.gene}-panel] {GLYCOGEN_MNXM} (glycogen) never became a "
                         f"node in this build -- cannot mark it on the panel")

    out = OUT_DIR / f"{args.gene}_delta_panel_fold{args.fold}_{args.orientation}_{args.element}.parquet"
    df.to_parquet(out)
    print(f"\n[{args.gene}-panel] {len(df)} metabolites with a delta; wrote {out}", file=sys.stderr)

    gly = df[df.mnxm == GLYCOGEN_MNXM].iloc[0]
    rank = int((df.delta.abs() >= abs(gly.delta)).sum())
    print(f"[{args.gene}-panel] glycogen delta = {gly.delta:.6g} (base {gly.draw_base:.6g}, "
          f"pert {gly.draw_pert:.6g}); rank {rank} of {len(df)} by |delta| "
          f"({rank/len(df):.4%} at or above it)", file=sys.stderr)


if __name__ == "__main__":
    main()
