...
def publish_by_type(results: Path, mapping: dict[str, str], dest_root: Path,
    ...
    Pass `record_provenance_at=<processed root>` when publishing a *reference*.
    The bytes carry no identity, and the copy is where the product's lineage id
    would otherwise be lost -- see `_record_publish_provenance`.
    """
