...
### 6. close the atom count before you answer

This is where most rewrites fail, so do it deliberately and do it last.

Once you have both sides, **count C, N, O, P and S on each side** and compare. The two
totals must be equal element by element. They usually are not on the first pass.

- **Oxygen short on one side?** Add `WATER` to that side. Most of these misses are
  a hydrolysis or condensation whose water was left implicit in the source equation.
- **Phosphorus short?** You dropped or duplicated a phosphate. Check that ATP -> ADP + Pi
  is written with all three species present.
- **Carbon short?** A leaving group went missing — CO2 (`MNXM13`) from a decarboxylation,
  formate, acetate. Put it back on the side it leaves from.
- **Still not equal?** You do not understand this reaction well enough to rewrite it.
  Set `"action":"refuse"`. Do not ship an equation whose atoms do not cancel — it is
  rejected downstream anyway, so a wrong rewrite and a refusal cost the same, and the
  refusal is honest.

Hydrogen and charge are *not* checked. Do not spend effort on them, and do not add protons
to fix an oxygen deficit.

#### worked case: the implicit water

    an aldehyde + NAD+ = a carboxylate + NADH

The oxidation puts an oxygen on the carbon, and it comes from water. Written with real
structures the count only closes with water on the left:

    acetaldehyde + NAD+ + WATER = acetate + NADH      C 2/2, O 3/3 with the water, 2/3 without

### 7. `working` comes first, and it is where you do the counting

The first field of your answer is `working`. Use it. Write the rewritten equation in plain
text and tally the heavy atoms on each side, then emit `left` and `right` to match what you
worked out. Keep it under 400 characters — it is a scratchpad, not an explanation.

    working: "acetaldehyde + NAD+ + WATER = acetate + NADH; L C2 O2 +H2O -> C2 O3, R C2 O3. ok"
    working: "L C6 O6 P1, R C6 O5 P1 -- one O short on the right, add WATER left. now 7/7. ok"
    working: "acyl-carrier is NO STRUCTURE and the transfer needs its thiol; cannot stand in. refuse"

If the tally in `working` does not close, set `"action":"refuse"`. Writing a tally that
does not balance and then emitting the sides anyway is the worst available answer: it is
rejected downstream and it wastes the reasoning you just did.

## Rules
- Every `id` must be from the HAS STRUCTURE list, with two exceptions you may always
  use even when they are not listed: `WATER` and `MNXM13` (CO2). Never reference a
  NO STRUCTURE accession.
...
- Count C/N/O/P/S on both sides before answering, and refuse if they do not match.
- Do not balance hydrogen or charge; heavy atoms only.
- Keep `reason` and every `why` to one short clause. Long prose is truncated mid-answer
  and the whole rewrite is lost.
