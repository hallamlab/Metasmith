...
One reaction per line: `MNXR_ID  <left> = <right>`, where each term is
`[coefficient ]Name [MNXM_ACCESSION]`. `WATER` is water.

## Output

One JSON object per line, nothing else:

    {"mnxr":"MNXR123", "action":"rewrite"|"unchanged"|"refuse",
     "strategies":["algebra","chunk","polymer_once","protein_cap"],
     "left":  [{"n":1,"id":"MNXM1105941"}, {"n":1,"smiles":"CCOC(C)=O","as":"acetate ester stand-in"}],
     "right": [...],
     "dropped":[{"id":"MNXM8975","why":"generic acceptor, cancels both sides"}],
     "balanced":true, "confidence":"high"|"medium"|"low",
     "reason":"one clause naming the chemistry preserved"}

Each term is either `{"n":coeff,"id":"MNXM..."}` — reuse a database accession whose
structure is already fine — or `{"n":coeff,"smiles":"...","as":"short label"}` when you
introduce a stand-in. Coefficients are positive on both sides.

**`balanced` must be your own check that heavy atoms (C, N, O, P, S, and any metal) balance
left to right. Count them. Hydrogen is excluded — do not try to balance H or charge.**

## The strategies, and how to apply them

### 1. `algebra` — cancel the amorphous mass
An amorphous participant that appears on **both sides** carrying the same body transfers no
tracked atoms. Delete it from both sides and record it in `dropped`.
...
### 2. `chunk` — truncate the inert scaffold
...
### 3. `polymer_once` — run elongation exactly once
...
- If the polymer is inert (it appears unchanged on both sides), use `algebra` and cancel it
  instead — do not expand it.

### 4. `protein_cap` — cap the protein
...
Set `"action":"unchanged"` and echo the terms as `id` references. Do not "improve" a
reaction that already has structures for everything — that is the failure mode this
