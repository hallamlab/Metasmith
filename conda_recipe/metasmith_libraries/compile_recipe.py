...
PKG_ROOT = HERE.joinpath("../../src/metasmith_libraries").resolve()
sys.path = list(set([str(PKG_ROOT)] + sys.path))

# import constants from the module's own setup.py, not the repo-root one --
# metasmith_libraries builds as its own nested package, mirroring metasmith's
# own conda_recipe/compile_recipe.py but pointed at src/metasmith_libraries/.
from setup import NAME, VERSION, SHORT_SUMMARY

USER = "hallamlab"

# ======================================================
# parse dependencies
with open(HERE.joinpath("../../envs/metasmith_libraries/base.yml")) as y:
...
#
# built standalone from within src/metasmith_libraries/ (its own package
# root), so dist/ lands there rather than at the repo root.

dist_path = PKG_ROOT.joinpath("dist")
assert dist_path.exists(), f"did you forget to build the pip package first? (cd src/metasmith_libraries && python -m build)"
