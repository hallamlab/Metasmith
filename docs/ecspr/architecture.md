...
**One bake input is produced by the previous bake, and the cycle has to be closed deliberately.**
`aam_forecast` decides which reactions get an element-reduced submission built for them partly
from `prior_bake_logs` — which reaction Indigo hangs on is not a function of the string, so the
empirical half is simply what the last run wrote down. That makes the logs a `given` with no
producer *within* a run and a product of the route as a whole, which `ecspr.bake.aam.runlogs`
writes where the route ends. A generation that does not write its own leaves the next one reading
its grandparent's silences, and nothing says so: the forecast reports a denominator, not an age.
