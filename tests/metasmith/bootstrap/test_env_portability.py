"""Portability is a question the tooling answers, not one discovered by running.

Three pieces, matching the three moments the question can be asked:

  * `ScanSource` -- author time, from the source alone.
  * `check_env_declarations` -- `metasmith transform validate`, same scan plus
    the contract rules.
  * `_read_env_manifest` / `_check_env_portability` -- run time, against the
    agent that actually has to execute the thing.
"""
...
def test_validate_accepts_a_container_only_transform():
    # Container-only is a fact about the tool, not a defect. The preflight, not
    # validate, is where it becomes an error -- and only against a mamba agent.
...
def test_validate_says_it_is_syntactic_only():
    # A clean scan is the absence of a detected violation, not a proof.
...
def test_preflight_passes_a_fully_portable_plan():
    ...
    _check_env_portability(m, _mamba())  # no raise
...
def test_preflight_is_a_no_op_under_a_container_runtime():
    m = {"P1": _step("P1", "interproscan", ["ifContainerDo"], {"ipr.env": ["container"]})}
    _check_env_portability(m, _docker())  # no raise
...
def test_preflight_is_a_no_op_with_no_manifest():
    _check_env_portability({}, _mamba())  # no raise
...
class _CatShell:
    """Stands in for the agent shell: answers the manifest `cat` and nothing else."""
...
def test_missing_manifest_reads_as_nothing_to_check():
    # An older metasmith staged this workspace; that is indistinguishable from
    # "no step declares anything" and must not fail every existing run.
