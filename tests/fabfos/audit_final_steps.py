...
def confirm_junction(profiles, lengths, c, d, hardstop_frac=0.25):
    ...
    if la is None or ra is None:
        return False
    left_far = la[bd:].mean() / max(la[:bd].mean(), 1e-9)
    right_far = ra[:bd].mean() / max(ra[bd:].mean(), 1e-9)
...
def test_hardstop_gate_fixes_false_positive_without_losing_true_calls(profiles_lengths):
    ...
    assert confirmed["FF.13B03"] is True
    assert confirmed["FF.19C15"] is True
    assert confirmed["FF.08B18"] is False
...
def test_trim_shrinks_overassembly_and_spares_inwindow(profiles_lengths):
    ...
    assert all(lengths[c] <= HI for c in inwin)
