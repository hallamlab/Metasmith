"""Forward-looking xfail tests for lineage-addressed cache identity.

All tests in this file are landed as ``@pytest.mark.xfail(strict=True)``
keyed to the S-section that will turn them green. Once that section
lands, the xfail marker on the affected test(s) gets removed in the
same PR. ``strict=True`` ensures a test that starts passing before its
section is shipped fails loudly rather than silently turning green.

Coverage: G1 (lineage key shape), G8 (leaf AddItem unique), G9
(lineage-only identity is workspace-independent), G10 (cross-workspace
